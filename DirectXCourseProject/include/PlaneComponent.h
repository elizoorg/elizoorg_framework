...
class ENGINE_API PlaneComponent : public GameComponent {
private:
...
public:
  PlaneComponent(Engine::Application *app) : GameComponent(app) {
    ...
    for (int i = 0 - lineCount / 2; i < lineCount / 2; ++i) {
      points.push_back(Point({Vector4(static_cast<float>(i), 0.0f,
                                      -cellSize * lineCount / 2, 1.0f),
                              i % 5 == 0 ? boldColor : color}));
      indices.push_back(k++);
      points.push_back(Point(
          {Vector4(static_cast<float>(i), 0.0f, cellSize * lineCount / 2, 1.0f),
           i % 5 == 0 ? boldColor : color}));
      indices.push_back(k++);
    }
    for (int i = 0 - lineCount / 2; i < lineCount / 2; ++i) {
      points.push_back(Point({Vector4(-cellSize * lineCount / 2, 0.0f,
                                      static_cast<float>(i), 1.0f),
                              i % 5 == 0 ? boldColor : color}));
      indices.push_back(k++);
      points.push_back(Point(
          {Vector4(cellSize * lineCount / 2, 0.0f, static_cast<float>(i), 1.0f),
           i % 5 == 0 ? boldColor : color}));
      indices.push_back(k++);
    }
  };
